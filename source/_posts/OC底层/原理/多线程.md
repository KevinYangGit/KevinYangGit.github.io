...
可以看到打印结果里只有一条打印信息，这是因为加锁后再次调用 `-(void)recursiveMutexTest` 方法，会发现 _recursiveMutex 已经上锁了，线程会卡死。
...
可以看到打印结果里只有一条打印信息，这是因为两个方法使用的是同一把锁，在 `- (void)recursiveMutexTest` 方法加锁后再去调用 `- (void)recursiveMutexTest2` 方法时，会发现 _recursiveMutex 已经上锁了，线程会卡死。
...
使用递归锁后，死锁的问题不存在了。但是，加了递归锁后的打印结果跟没加锁一样，这样被加锁的代码块还安全吗？其实，递归锁只可以在当前线程重复加锁。也就是说，当前线程加锁后，当前线程再调用 `- (void)recursiveMutexTest` 方法还可以重复加锁，而其它线程在调用 `- (void)recursiveMutexTest` 方法时判断到已经加过锁了就不再加锁了，会阻塞住线程。
