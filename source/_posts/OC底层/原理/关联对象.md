...
tags: OC底层原理
---
* 思考：Category 能否添加成员变量？如果可以，如何给 Category 添加成员变量？
* 类对象可以关联对象吗？
* 关联对象存储在什么位置？
* 对象及其关联对象之间有强引用关系吗？
* 对象释放后，关联对象会被释放吗？
...
通过定义全局变量 _weight 的方式可以实现成员变量的效果，但是 _weight 是全局变量，所有的实例对象 persion 会共用同一个 _weight。这样会导致所有的实例对象 persion 都可以修改 _weight，无法保证单个实例对象 persion 与 _weight 一一对应关系。
...
👆通过 OBJC_ASSOCIATION_ASSIGN 策略关联了对象 persion3，此时在 ObjcAssociation 对象里的 value 记住的是关联对象 persion3 的地址值，当关联对象 persion3 离开大括号销毁时，value 依然保存着关联对象的地址值。如果这个时候再通过 value 去访问关联对象就会出现坏内存访问的错误。
